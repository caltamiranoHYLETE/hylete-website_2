...
$content = "<h1>1. Creating a contact in the address book</h1>";
...
try {		    
    ...
    $content .= "<br>Created contact with id: " . $contactId;
} catch (Exception $e) {
    $content .= "<br>Caught exception when creating contact : " .  $e->getMessage() . "<br>";
}

$content .= "<h1>2. Getting the contact by its phonenumber and updating it.</h1>";
try {		    
    $contactjson = $pest->get('/contacts/phonenumber/' . $phonenumber);
    $contactarray = json_decode($contactjson,true);
    $content .= "<br>Retrieved contact with id: " .  $contactarray[ContactId] . ", should be identical with " . $contactId . " from part 1.<br>";
    ...
    $content .= "<br>Updated contact<br>";
} catch (Exception $e) {
    $content .= "<br>Caught exception when retrieving or updating contact : " .  $e->getMessage() . "<br>";
}

$content .= "<h1>3. Sending a message to the contact</h1>";
try {		    
    $messagedata = array(
    ...
    $content .= "<br>Message was sent! MessageId: " .  $messagearray[MessageId];
} catch (Exception $e) {
    $content .= "<br>Caught exception when sending message : " .  $e->getMessage() . "<br>";
}

$content .= "<h1>4. Deleting contact.</h1>";
try {		    
    $pest->delete('/contacts/' . $contactId);
    $content .= "<br>Deleted contact.<br>";
} catch (Exception $e) {
    $content .= "<br>Caught exception when deleting contact : " .  $e->getMessage() . "<br>";
}
