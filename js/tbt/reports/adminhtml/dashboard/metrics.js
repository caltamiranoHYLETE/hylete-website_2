 ...
 */
sweettooth.Dashboard.Metrics = {
    updateReport: function(selectIdentifier, reportCode, reportUrl, containerObj) {
        new Ajax.Request(reportUrl, {
            parameters: {
                report_code: reportCode,
                period: selectIdentifier.getValue()
            },
            asynchronous: true,
            evalJS: true,
            onSuccess: function(transport) {
                var response = transport.responseText;
                selectIdentifier.up('div.' + containerObj).replace(response);
            }.bind(this)
        });
