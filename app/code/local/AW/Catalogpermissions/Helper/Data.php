...
class AW_Catalogpermissions_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public static function prepareRewrites()
    {
        ...
        /* Add rewrite for native layered navigation */
        if (!Mage::getStoreConfig('advanced/modules_disable_output/AW_Catalogpermissions')) {
            $node = Mage::getConfig()->getNode('global/models/catalog_resource/rewrite');
            $dnode = Mage::getConfig()->getNode('global/models/catalog_resource/drewrite/layer_filter_attribute');
    ...
    public static function checkProductAvailability($hiddenProducts = array(), $requestItem = 'product')
    {
        ...
        if (is_array($superGroup)) {
            if (count($superGroup) > 0) {
                ...
                $superGroupKeys = array_keys($superGroup);

                foreach ($superGroupKeys as $productId) {
                ...
                Mage::app()->getRequest()->setParam('super_group', $superGroup);
            }
        }
        ...
        if (is_array($superGroup)) {
            if (count($superGroup) > 0) {
                ...
                $superGroupArray = $superGroup;
                foreach ($superGroupArray as $key => $productId) {
                ...
                Mage::app()->getRequest()->setParam('super_attribute', $superGroup);
            }
        }
