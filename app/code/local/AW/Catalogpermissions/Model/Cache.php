...
class AW_Catalogpermissions_Model_Cache extends Mage_Core_Model_Abstract
{
    ...
    const BUNDLED_CACHE_TAG = 'BUNDLED';
    ...
    public function refreshCache()
    {
        ...
        /* Cache product ids with hidden price for every customerGroup x storeViews */
        $scopeInfo = array();
        foreach ($this->_getCustomerGroups() as $group) {
            foreach ($this->_getStoresCollection() as $store) {
                ...
                $this->_cacheBundledProductsPrice($scopeInfo);
            }
        }
    ...
    protected function _cacheBundledProductsPrice(array $scope)
    {
        $_cacheTag = $this->_getCacheTag($scope['groupId'], $scope['storeId'], self::BUNDLED_CACHE_TAG);
