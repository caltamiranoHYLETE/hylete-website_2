...
class Nosto_Tagging_Helper_Price extends Mage_Core_Helper_Abstract
{
    ...
    public function getItemFinalPriceInclTax(Mage_Sales_Model_Order_Item $item)
    {
        ...
        if($orderCurrencyCode != $baseCurrencyCode && $item->getOrder()->getShippingMethod() == 'globale_standard') {
            // in the case of Global-e order the price was allready calculated.
            $priceInOrderCurrency = $item->getRowTotal() + $item->getTaxAmount()
            + $item->getHiddenTaxAmount() - $item->getDiscountAmount();
        }
        elseif ($orderCurrencyCode != $baseCurrencyCode) {
