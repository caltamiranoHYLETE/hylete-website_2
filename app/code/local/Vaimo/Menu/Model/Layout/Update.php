...
class Vaimo_Menu_Model_Layout_Update extends Vaimo_Menu_Model_Abstract
{
    ...
    {
        $cacheKey = $this->_getCacheKey($update, $package, $theme);
        $cache = $this->getFactory()->getHelper('vaimo_menu/cache');
        ...
        if (!$this->_shouldReload($cacheKey)) {
            ...
            $dbUpdates = $this->getFactory()->getModel('vaimo_menu/layout_db_update');

            $updates = $dbUpdates->getDbUpdatesForPackageAndTheme($update, $package, $theme);
            $updates = $this->_diffLayoutUpdates($updates, $updateBefore);
    ...
    protected function _getCacheKey($update, $package, $theme)
    {
        return self::CACHE_TAG . '_' . $package . '_' . $theme . '_' . $update->getCacheId();
    }
