 ...
 */
abstract class Icommerce_Scheduler_Helper_Operationrunner_Abstract extends Mage_Core_Helper_Abstract
{
    /**
    ...
    protected function _getFileUrlToRun($operation)
    {
        ...
        $schedulerHelper = Mage::helper('scheduler');
        ...
        $globalCustomUrl = (string)Mage::getConfig()->getNode(
    ...
    protected function _updateOperationStatus($operation, $result, $createdAt, $runChildren, $verbose)
    {
        ...
        if ($runChildren && $operation->hasChildOperations()) {
            $operation->setStatus(Icommerce_Scheduler_Helper_Data::OPERATION_STATUS_WAITING);
        } else {
            $operation->setStatus(Icommerce_Scheduler_Helper_Data::OPERATION_STATUS_ENABLED);
        }

        if ($result['status'] != Icommerce_Scheduler_Helper_Data::HISTORY_STATUS_NOTHING_TO_DO
        ...
        if ($history->getStatus() == Icommerce_Scheduler_Helper_Data::HISTORY_STATUS_FAILED and $operation->getRerun() == 1) {
        ...
        try {
            if ($operation->getEmailEnabled()) {
                ...
                if (in_array($history->getStatus(), $emailStatus)) {
                    ...
                    $email = Mage::getModel('scheduler/email');
