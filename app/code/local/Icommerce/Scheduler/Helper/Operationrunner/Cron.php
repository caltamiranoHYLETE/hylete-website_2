...
class Icommerce_Scheduler_Helper_Operationrunner_Cron extends Icommerce_Scheduler_Helper_Operationrunner_Abstract
{
    /**
    ...
    public function runOperation($operation, $runChildren = false, $verbose = false)
    {
        ...
        try {
            ...
            $status = Icommerce_Scheduler_Helper_Data::HISTORY_STATUS_FAILED;
            $message = $e->getMessage();
            $result = isset($result['result']) ? $result['result'] : '';
            $this->_saveHistory($operation, $status, $createdAt, $message, $result);
            $operation->setStatus(Icommerce_Scheduler_Helper_Data::OPERATION_STATUS_DISABLED);
    ...
    {
        $tempFile = Mage::getBaseDir('tmp') . DS . 'scheduler_' . getmypid() . '.log';
    ...
    {
        $result = array(
            'status' => Icommerce_Scheduler_Helper_Data::HISTORY_STATUS_NONE,
            'message' => '',
            'memory_usage' => 0,
            'result' => '',
            'messages' => array(),
