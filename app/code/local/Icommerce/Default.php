...
class Icommerce_Default {
    ...
    static function getLoadModelData( Mage_Core_Model_Abstract $obj, $attr ){
        $data = $obj->getData();
        if( isset($data[$attr]) ) return $data[$attr];
    ...
    static function isEnterprise(){
        static $stEnterprise;
        if( $stEnterprise===null ) $stEnterprise = (bool)Mage::getConfig()->getModuleConfig('Enterprise_Enterprise');
        return $stEnterprise;
    }
    ...
    /**
     * Returns config setting based on admin scope. Uses the current admin scope if $scope and $scope_id are not provided.
     *
     * @param string $path
     * @param string $scope
     * @param int $scope_id
     * @return string|null Config value based on admin scope or NULL if no value was found.
     */
    static function getScopeConfig($path, $scope = null, $scope_id = null) {
        if (is_null($scope)) {
            $scope = self::getAdminScope();
        }
        if (is_null($scope_id)) {
            $scope_id = self::getAdminScopeId();
        }

        $store_id = $website_id = null;
        if ($scope == 'stores') {
            $store_id = $scope_id;
        ...
        $rows = Icommerce_Db::getRows('SELECT scope, scope_id, value FROM core_config_data WHERE path = ?', array($path));

        $storeValue = $websiteValue = $defaultValue = null;
        foreach ($rows as $row) {
            if ($row['scope'] == 'stores' && $row['scope_id'] == $store_id) {
                $storeValue = $row['value'];
            }
            elseif ($row['scope'] == 'websites' && $row['scope_id'] == $website_id) {
                $websiteValue = $row['value'];
            }
    ...
    static function getMetaDir(){
        static $st_meta_dir;
        if( !$st_meta_dir ){
            $st_meta_dir = isset($_SERVER["meta_path"]) ? $_SERVER["meta_path"] : "media/meta";
        }
        return $st_meta_dir;
    }

    static function isLiveSite(){
        $v = @file_get_contents( self::getMetaDir() . "/is_live" );
        return $v && (int)$v;
    }
    ...
    static function getCategoryIdByUrl(){
        static $st_cat_id;
        if( $st_cat_id===null ){
            $st_cat_id = false;
            $url_offset = self::getCurrentSiteUrl(false,true);
            if( ($p = strpos( $url_offset, "/" ))!==FALSE && strlen($url_offset)>$p ){
                $url = substr($url_offset,$p+1);

                // Clean controller/action URL ?
                if( ($pp=strpos($url,"catalog/category/view/id/"))!==false ){
                    // Have ID here
                    $st_cat_id = (string)(int)substr($url,$pp+25);
                } else {
                    // Enterprise 1.13 changed this around - new table + URL:s never being shared between stores
                    if( Icommerce_Db::tableExists("enterprise_url_rewrite") ){
                        if( $path = Icommerce_Db::getValue( "SELECT target_path FROM enterprise_url_rewrite WHERE request_path=?", array($url) ) ){
                            $matches = array();
                            preg_match( "@.*/([0-9]+)$@", $path, $matches );
                            if( count($matches)>1 ) $st_cat_id = $matches[1];
                        }
                    } else {
                        $store_id = self::getStoreIdFromUrl( );
                        if( $store_id ){
                            $st_cat_id = Icommerce_Db::getValue( "SELECT category_id FROM core_url_rewrite WHERE request_path=?
                                                                  AND store_id=?", array($url,$store_id) );
                        } else {
                            $st_cat_id = Icommerce_Db::getValue( "SELECT category_id FROM core_url_rewrite WHERE request_path=?", array($url) );
                        }
                    }
                }

                if( $st_cat_id===null ) $st_cat_id = false;
            }
        }
        return $st_cat_id;
    }
    ...
    */
    ...
    /**
     * Returns the current admin scope.
     *
     * @return string
     */
    static function getAdminScope() {
        return self::getAdminScopeAbstract();
    }

    /**
     * Returns the current admin scope ID.
     *
     * @return int
     */
    static function getAdminScopeId() {
        return self::getAdminScopeAbstract(true);
    }

    /**
     * Returns the current admin scope or admin scope ID.
     *
     * @param bool $isScopeId Return either scope ID or scope
     * @return int|string
     */
    static function getAdminScopeAbstract($isScopeId = false) {
        $store = Mage::app()->getRequest()->getParam('store');
        $website = Mage::app()->getRequest()->getParam('website');

        if ($store) {
            return $isScopeId ? Mage::getModel('core/store')->getCollection()->addFieldToFilter('code', $store)->getFirstItem()->getStoreId() : 'stores';
        }
        elseif ($website) {
            return $isScopeId ? Mage::getModel('core/website')->getCollection()->addFieldToFilter('code', $website)->getFirstItem()->getWebsiteId() : 'websites';
        }
        else {
            return $isScopeId ? 0 : 'default';
        }
    }
    ...
    static function formatValue( $val, $attrib, $store_id=null, $entity_type="catalog_product" ){
        ...
        if( $ainfo["frontend_input"]=="select" ){
            ...
            static $st_format;
            if( !$st_locale ){
                $st_locale = Mage::getSingleton("core/locale");
