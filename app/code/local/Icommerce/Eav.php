...
class Icommerce_Eav {
    ...
    const VAIMO_EAV_CACHE_KEY = "vaimo_eav_data";
    const VAIMO_EAV_CACHE_LIFETIME = 14400;
    ...
    /**
     * @var An array that contains necessary entity + attribute info, to avoid looking up this information from DB.
     */
    static $_cached_eav_data;

    /**
     * Save cached entity + attribute info to cache, and update internal PHP var with new data.
     * @static
     * @param $data
     */
    static protected function saveEntityAttributeInfoToCache( $data ){
        $cache = Mage::app()->getCache();
        $cache->save( serialize($data), self::VAIMO_EAV_CACHE_KEY, array("CONFIG"), self::VAIMO_EAV_CACHE_LIFETIME );
        self::$_cached_eav_data = $data;
    }

    /**
     * Read cached entity/attribute information from cache or create an array with basic information, if not in cache yet.
     * @static
     * @return
     */
    static protected function getCachedEntityAttributeInfo(){
        if( self::$_cached_eav_data===null ){
            $cache = Mage::app()->getCache();
            if( !($sdata=$cache->load(self::VAIMO_EAV_CACHE_KEY)) ){
                $data = array();
                //Enterprise always had this column, but CE < 1.4 did not
                if (Icommerce_Default::getMagentoVersion() < 1400) {
                    $fields = 'entity_type_id, entity_type_code, entity_table';
                } else {
                    $fields = 'entity_type_id, entity_type_code, entity_table, additional_attribute_table';
                }

                $data["eav_entity_by_id"] = Icommerce_Db::getAssociative("SELECT ".$fields." FROM eav_entity_type");
                $data["eav_entity_by_code"] = array();
                foreach( $data["eav_entity_by_id"] as $id => &$vals ){
                    $data["eav_entity_by_code"][$vals["entity_type_code"]] = $id;
                    $table = str_replace("/","_",$vals["entity_table"]);
                    if( !strendswith($table,"_entity") ) $table .= "_entity";
                    $vals["table"] = $table;
                    $entity_table_varchar = $table."_varchar";
                    $vals["only_global"] = (bool)(!Icommerce_Db::tableExists($entity_table_varchar) || !Icommerce_Db::columnExists($entity_table_varchar,"store_id"));
                    $vals["backend_types"] = Icommerce_Db::getColumn( 'SELECT DISTINCT(backend_type) FROM `eav_attribute` WHERE entity_type_id=?',array($id) );
                }
                $data["eav_attributes"] = array();
                self::saveEntityAttributeInfoToCache( $data );
            } else {
                self::$_cached_eav_data = unserialize( $sdata );
            }
        }
        return self::$_cached_eav_data;
    }


    /**
     * Cache information about another attribute
     * @static
     * @param $entity_type_id
     * @param $attrib_code
     * @param $attr_info
     */
    static protected function cacheAttributeInfo( $entity_type_id, $attrib_code, $attr_info ){
        // Force a re-read of cached value, other processes might have modified while we were working
        self::$_cached_eav_data = null;
        $info = self::getCachedEntityAttributeInfo();
        $info["eav_attributes"][$entity_type_id][$attrib_code] = $attr_info;
        $info["eav_attributes"]["by_id"][$attr_info["attribute_id"]] = $attrib_code;
        self::saveEntityAttributeInfoToCache( $info );
    }
    ...
    static function getEntityTypeId( $entity_type_code ){
        ...
        $info = self::getCachedEntityAttributeInfo();
        return isset($info["eav_entity_by_code"][$entity_type_code]) ? $info["eav_entity_by_code"][$entity_type_code] : null;
    }
     ...
     * @return int|null entity_type_id (null if not found)
     */
    static function getEntityTypeCode( $entity_type_id ){
        if( !$entity_type_id ) {
            return null;
        }

        if( !Icommerce_Utils::isInteger($entity_type_id) ){
            return $entity_type_id;
        }

        $info = self::getCachedEntityAttributeInfo();
        return isset($info["eav_entity_by_id"][$entity_type_id]) ? $info["eav_entity_by_id"][$entity_type_id]["entity_type_code"] : null;
    }
        ...
        if( $id_or_object instanceof Varien_Object ){
            ...
            if( !$entity_type ){
                ...
                if( !($entity_type_id=$obj->getData("entity_type_id")) ){
                    ...
                    $info = self::getCachedEntityAttributeInfo();
                    $entity_type = isset($info["eav_entity_by_id"][$entity_type_id]) ? $info["eav_entity_by_id"][$entity_type_id]["entity_type_code"] : null;
                }
            }
        } else {
    ...
    static function getEntityAdditionalTable( $entity_type_code ){
        ...
        if( !($eid = self::getEntityTypeId($entity_type_code)) ) return null;
        $info = self::getCachedEntityAttributeInfo();
        if( isset($info["eav_entity_by_id"][$eid]) ){
            if( !($additional_table = $info["eav_entity_by_id"][$eid]["additional_attribute_table"]) ) return null;
            // Remove the slash in there
            return str_replace( "/", "_", $additional_table );
        } else {
            return null;
        }
    }
    ...
    static function getAttributeInfo( $attrib_code, $entity_type_code=null ){
        ...
        $info = self::getCachedEntityAttributeInfo();
        if( isset($info["eav_attributes"][$entity_type_id][$attrib_code]) ){
            return $info["eav_attributes"][$entity_type_id][$attrib_code];
        }

        $sql = 'SELECT attribute_id, backend_type, entity_type_id, frontend_input, source_model, backend_table FROM eav_attribute WHERE attribute_code=?';
        ...
        self::cacheAttributeInfo( $entity_type_id, $attrib_code, $attr_info );
        return $attr_info;
    }
    ...
    static function getAttributeCode($attrib_id){
        $info = self::getCachedEntityAttributeInfo();
        if( isset($info["eav_attributes"]["by_id"][$attrib_id]) ){
            return $info["eav_attributes"]["by_id"][$attrib_id];
        } else {
            $info = Icommerce_Db::getRow( 'SELECT attribute_code, entity_type_id FROM eav_attribute WHERE attribute_id=?', array($attrib_id) );
            if( is_null($info) ){
                throw new Exception('Icommerce_Default - Failed looking up EAV attribute code');
            }

            // Make sure this info is cached, by loading info
            self::getAttributeInfo( $info["attribute_code"], $info["entity_type_id"] );
            return $info["attribute_code"];
        }
    }
    ...
    static function isDetailTable($tableName,$backend_type=null)
    {
        ...
        switch ($tableName) {
            ...
            case 'catalog_category_entity_url_key':
            case 'catalog_product_entity_url_key':
                return true;
        }
        return false;
    }

    static function getBackendTable($entity_type_code,$attrib_code)
    {
        $attrInfo = self::getAttributeInfo( $attrib_code, $entity_type_code );
        return $attrInfo ? $attrInfo['backend_table'] : null;
    }
    ...
    static function getEntityMetaData( $entity_type ){
        if( !($entity_type_id = self::getEntityTypeId($entity_type)) ) return null;

        $info = self::getCachedEntityAttributeInfo();
        if( !isset($info["eav_entity_by_id"][$entity_type_id]) ) return null;;
        return $info["eav_entity_by_id"][$entity_type_id];
    }
    ...
    static function createEavAttribute(
        ...
        unset( self::$_cached_eav_data["eav_attributes"][$entity_type][$new_attr_code] );
        ...
        // Do we have additional attribute info to set ?
        if( $add_tbl=self::getEntityAdditionalTable($entity_type) ){
    ...
    static function deleteEavAttribute( $acode, $entity_type ){
        if( !$entity_type ) return null;
        if( !($ainfo = self::getAttributeInfo( $acode, $entity_type )) ){
            return null;
        }
        $attribute_id = $ainfo["attribute_id"];
        Icommerce_Db::write( "DELETE FROM eav_attribute WHERE attribute_id=?", array($attribute_id) );
        if( $additional_table = self::getEntityAdditionalTable( $entity_type ) ){
            Icommerce_Db::write( "DELETE FROM $additional_table WHERE attribute_id=?", array($attribute_id) );
        }

        // Make sure we don't have it cached anymore
        if( self::$_cached_eav_data ){
            $entity_type_id = self::getEntityTypeId($entity_type);
            $info = self::$_cached_eav_data;
            unset( $info["eav_attributes"][$entity_type_id][$acode] );
            unset( $info["eav_attributes"]["by_id"][$attribute_id] );
            self::saveEntityAttributeInfoToCache( $info );
        }
    ...
    static function writeEavValue( $entity_id, $key, $val, $entity_type=null, $store_id=null ){
        ...
        if( $obj ) self::setDataWithoutChange( $obj, $key, $val );
