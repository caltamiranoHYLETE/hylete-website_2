...
class Cryozonic_Stripe_Model_PaymentIntent
{
    ...
    protected function loadFromCache($quote)
    {
        if (empty($quote))
            return null;
        ...
        if (!empty($object))
            $this->paymentIntent = unserialize($object);
        else
            return null;
    ...
    protected function updateCache($quoteId)
    {
        ...
        $data = serialize($this->paymentIntent);
    ...
    public function shouldUsePaymentIntents()
    {
        ...
        // $isAuthorizeOnly = $this->helper->isAuthorizeOnly();
    ...
    {
        return ($this->paymentIntent->status == Cryozonic_Stripe_Model_PaymentIntent::CAPTURED ||
            $this->paymentIntent->status == Cryozonic_Stripe_Model_PaymentIntent::AUTHORIZED);
    }
    ...
    {
        $this->create($order->getQuote(), $payment); // Load or create the Payment Intent
        ...
        if (!$this->isSuccessful())
        {
            ...
            {
                if ($this->helper->isAdmin())
                    Mage::throwException($this->helper->__("This card cannot be used because it requires a 3D Secure authentication by the customer."));

                if ($this->helper->isMultiShipping())
                    Mage::throwException($this->helper->__("This card cannot be used for multi-shipping orders because it requires 3D Secure authentication." .
                        " Please place your order on its own for this shipping address, or use a different card."));

                Mage::throwException("Authentication Required: " . $this->getClientSecret());
            }

            if (!$this->isSuccessfulStatus())
                Mage::throwException($this->helper->__("Could not confirm payment intent"));
        }
    ...
    public function setPaymentMethod($paymentMethodId, $save = false)
    {
        ...
        if ($save && (!isset($this->paymentIntent->save_payment_method) || !$this->paymentIntent->save_payment_method))
        {
            ...
            if (!$this->helper->findCardByPaymentMethodId($paymentMethodId))
            {
