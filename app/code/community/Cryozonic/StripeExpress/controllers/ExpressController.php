...
class Cryozonic_StripeExpress_ExpressController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function estimate_cartAction()
    {
        ...
        $this->getResponse()->setBody(Zend_Json::encode([
            "paymentIntent" => Mage::getSingleton("cryozonic_stripe/paymentIntent")->create()->getClientSecret(),
            "results" => $result
        ]));
    }
    ...
    {
        $this->stripe = $this->getHelper()->getStripe();
        $this->store = $this->stripe->getStore();
        $this->saveCards = $this->store->getConfig('payment/cryozonic_stripeexpress/ccsave');
        ...
        Mage::getSingleton("cryozonic_stripe/paymentIntent")->create()->stopUpdatesForThisSession = true;
        ...
        try {
            // Set Payment Method
            $quote->setPaymentMethod('cryozonic_stripe');
            ...
            // Get Amount
            $use_store_currency = Mage::getStoreConfig('payment/cryozonic_stripe/use_store_currency');
            ...
            $quote->getPayment()->setAdditionalInformation('customer_stripe_id', $this->stripe->getCustomerStripeId());
            $quote->getPayment()->importData(array('method' => 'cryozonic_stripe'));

            // Save Quote
            $quote->save();

            // Place Order
            Mage::getSingleton("cryozonic_stripe/paymentIntent")->setPaymentMethod($paymentMethodId);
    ...
    public function addtocartAction()
    {
        ...
        try {
            ...
            Mage::dispatchEvent('cryozonic_stripeexpress_before_add_to_cart', array('product' => $product, 'request' => $request));
            ...
            if (!$isUpdated)
            {
                Mage::helper("cryozonic_stripeexpress/cart")->addToCart($product, $request);
            }
    ...
    /**
     * Get StripeExpress Helper
     * @return Cryozonic_StripeExpress_Helper_Data
     */
    public function getHelper()
    {
        return Mage::helper('cryozonic_stripeexpress');
    }
}
