    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        foreach ($ratearray as $rate) {
            if (!empty($rate) && $rate['price'] >= 0) {
                ...
                // BEGIN Vaimo change
                $code = 'matrixrate_' . $rate['pk'];
                // If we have set an explicit code on this method, then use that, otherwise fall back on original code
                if (isset($rate['code']) && $rate['code'] != "") {
                    $code = $rate['code'];
                }
                ...
                if (isset($rate['logo'])) {
                    $method->setLogo($rate['logo']);
                }
                $rate['description'] = isset($rate['description']) ? $rate['description'] : '';
                ...
                $method->setDescription($rate['description']);
