...
class Webshopapps_Shippingoverride2_Model_Shipping_Shipping extends Mage_Shipping_Model_Shipping
{
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        $override2Groups = $override2ResourceModel->getNewRate($request, $exclusionList, $customError, $weightIncArr,
            $minPriceArr, $productShipPrice);
        $override = true;
        $resultSet = array();
        if (empty($override2Groups) && count($exclusionList) < 1 && count($minPriceArr)< 1 && $productShipPrice == 0) {
            $override = false;
        }
        ...
        if (!$limitCarrier) {
            ...
            foreach ($carriers as $carrierCode => $carrierConfig) {
                if ($override) {
                    $resultSet[] = $this->_collectOverrideRates($carrierCode, $request, $exclusionList,$minPriceArr,
                        $override2Groups, $customError, $freightName, $freightConfig, $weightIncArr, $productShipPrice);
     ...
     * Note: weightIncArr and MinPriceArr() do not work inside dropship
     *
     * NOTE: If change this interface make sure you alter Dropship else will break it
     *
     * @param String $origRequest
     * @param String $request
     * @param String $exclusionList
     * @param String $deliveryOverrideList
     * @param        $customError
     * @param string $freightName
     * @param string $freightConfig
     * @param array  $weightIncArr
     * @param int    $productShipPrice
     * @internal param String $totalShipPrice
     * @return Webshopapps_Shippingoverride2_Model_Shipping_Shipping
     */
    public function collectSpecialRates($origRequest, $request, $exclusionList, $minPriceArr, $deliveryOverrideList,
                                        $customError, $freightName = '', $freightConfig = '',
                                        $weightIncArr = array(), $productShipPrice = 0)
    {
        $resultSet = array();
        ...
        if (!$limitCarrier) {
            ...
            foreach ($carriers as $carrierCode => $carrierConfig) {
                ...
                $resultSet[] = $this->_collectOverrideRates($carrierCode, $request, $exclusionList,$minPriceArr,
                                                            $deliveryOverrideList, $customError, $freightName,
                                                            $freightConfig, $weightIncArr, $productShipPrice);
            }
        } else {
            if (!is_array($limitCarrier)) {
                $limitCarrier = array($limitCarrier);
    ...
    protected function _getLoadedMethods($rates)
    {

        $shippingMethodArr = array();
        foreach ($rates as $rate) {
            $shippingMethodArr[] = $rate->getMethodTitle();
        }

        return $shippingMethodArr;
    }

    protected function _collectOverrideRates($carrierCode, $request, $exclusionList, $minPriceArr,
                                             ...
                                             $weightIncArr = array(), $productShipPrice = 0)
    {
        ...
        $loadedShippingMethods = $this->_getLoadedMethods($rates);
        ...
        foreach ($deliveryOverrideList as $deliveryType => $override2Details) {
            ...
            try {
                if ($this->_useRegularExpressions) { //SO-52
                    foreach ($loadedShippingMethods as $methodTitle) {
                        if(preg_match("/" . $deliveryType . "/", $methodTitle)){
                            $regMatched = true;
                        }
                    }
                }
            } catch (Exception $e) {
                Mage::helper('wsalogger/log')->postDebug('shippingoverride2', 'Fatal Error', 'Pattern Matching Error');
            }

            if ($deliveryType != 'ALL' && !in_array($deliveryType, $loadedShippingMethods) && !$regMatched) {
                continue;
            }
            ...
            if (Mage::helper('wsacommon/shipping')->hasFreightCarrierEnabled() && $carrierCode == $freightName
                ...
                if (self::$_debug) {
                    Mage::helper('wsalogger/log')->postDebug('shippingoverride2', 'Exiting as weight not met',
                        $override2Details['cart_details']['weight']);
                }
                return array();
            }
            ...
            $this->_override2Request->setPackagePhysicalValue($override2Details['cart_details']['price']);//SO-30
        ...
        foreach ($rates as $key => $rate) {
            ...
            foreach ($deliveryOverrideList as $deliveryType => $override2Details) {
                ...
                if ($regMatched || (trim($rate->getMethodTitle()) == $deliveryType) || $deliveryType == 'ALL') {
                    $found = true;
                    if (self::$_debug) {
                    ...
                    if (array_key_exists('group_results', $override2Details) && !$override2Details['override']) {
                        ...
                        if (!$override2Details['wipe_rate']) {
                            ...
                            if (!is_numeric($newPrice)) {
                                if (self::$_debug) {
                                    Mage::helper('wsalogger/log')->postDebug('shippingoverride2', 'Couldnt match rate on 2nd request', $deliveryType);
                                }
                                $found = false; // no valid rate, reset
                            } else if ($newPrice > 0) {
            ...
            if ($found && $wipeRate && !empty($rates[$key])) {
                ...
                if ($percentageRate != -99 && $rates[$key]->getNewPrice() > 0) {
                    ...
                    $rates[$key]->setPrice($rates[$key]->getNewPrice() + ($rates[$key]->getPrice() * ($percentageRate / 100)));
                } else {
                    if ($rates[$key]->getNewPrice() == 0) {
                        $this->_freeMethods[$rates[$key]->getMethod()] = $rates[$key]->getPrice();
                    }
        ...
        foreach ($rates as $key => $rate) {
            if (!empty($rates[$key])) {
                $methodTitle = trim($rate->getMethodTitle());
                if (array_key_exists($methodTitle, $minPriceArr) && $rate->getPrice()<$minPriceArr[$methodTitle]) {
                    $rates[$key] = "";
                    continue;
                }
                ...
                // now check if no csv then add on the shipping price
                if ($productShipPrice != 0) {
                    $rate->setPrice($rate->getPrice() + $productShipPrice);
                }
                if ($rate->getPrice()<0) {
                    $rate->setPrice(0);
                }
