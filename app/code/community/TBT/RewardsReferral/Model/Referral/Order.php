...
class TBT_RewardsReferral_Model_Referral_Order extends TBT_RewardsReferral_Model_Referral_Abstract
{
    ...
    public function getReferralPointsForOrder($ro_rule, $order)
    {
        ...
        if ($simpleAction == 'by_fixed') {
            ...
            $full_earning    = Mage::getModel('rewards/points')->set($order->getTotalEarnedPoints(true));
    ...
    {
        return Mage::helper('rewards/transfer_reason')->getReasonId('referral_order');
    }
    ...
    public function hasOrderPendingTransfers()
    {
        ...
        $transfersCollection->addFieldToFilter(
                'reason_id', Mage::helper('rewards/transfer_reason')->getReasonId('referral_order')
            )
