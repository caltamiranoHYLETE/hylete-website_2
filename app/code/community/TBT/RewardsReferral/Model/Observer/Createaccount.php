...
class TBT_RewardsReferral_Model_Observer_Createaccount extends Varien_Object
{
    ...
    /**
     * Validates the referral email on the checkout
     * 
     * @param Varien_Event_Observer $event
     * @throws Exception
     * @event controller_action_postdispatch_checkout_onepage_saveBilling
     */
    public function afterSaveBilling($event)
    {
        $action = $event->getControllerAction();
        $request = $action->getRequest();
        $data = $request->getPost();
        
        if (!isset($data['billing']) || !isset($data['billing']['rewards_referral'])) {
            return $this;
        }
        
        $referralCodeOrEmail = $data['billing']['rewards_referral'];
        $referralEmail = Mage::helper('rewardsref/code')->parseEmailFromReferralString($referralCodeOrEmail);
        
        if (!$referralCodeOrEmail) {
            return $this;
        }
        
        try {
            $customerModel = Mage::getModel('customer/customer')->setWebsiteId(Mage::app()->getStore()->getWebsiteId());
            $referredCustomer = $customerModel->loadByEmail($referralEmail);
            
            if (!$referredCustomer->getId()) {
                $message = Mage::helper('rewardsref')->__("Referral email not yet registered on the store.");
            ...
            //Overriding the response object
    ...
    */
     ...
     * Event: sales_quote_save_before | rewards_update_catalog_points
     *
     * @param $observer
     * @return TBT_RewardsReferral_Model_Observer_Createaccount
     */
