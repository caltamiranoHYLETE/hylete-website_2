...
/**
 * Metrics Collection Helper for Revenue From Loyalty Customers
 *
 * @category   TBT
 * @package    TBT_Reports
 * @author     Sweet Tooth Inc. <support@sweettoothrewards.com>
 */
class TBT_Reports_Helper_Adminhtml_Metrics_Revenue_Loyalty
    extends Mage_Adminhtml_Helper_Dashboard_Abstract
{
    ...
    protected function _initCollection()
    {
        ...
        $this->_collection = Mage::getResourceModel('tbtreports/order_collection')
            ->onlyCompleteOrders()
            ->onlyOrdersByLoyaltyCustomers(false)
        ...
        if ($firstIndexedOrder) {
            if (strtotime($firstIndexedOrder['created_at']) > strtotime($dateFrom)) {
                ...
                $message = Mage::helper('tbtreports')->__(
                    "Loyalty revenue data has been available after %s only.", $orderCreatedAt
                );
            }
        } else {
            ...
            $message = Mage::helper('tbtreports')->__(
                "We don't have enough loyalty revenue data available just yet. Check again later."
            );
