...
class TBT_Rewards_Model_Tag_Observer extends Varien_Object {
    ...
    public function afterLoadTag(Varien_Event_Observer $o) {
        ...
        $this->oldData[$tag->getId()] = $tag->getData();
        return $this;
    }
    ...
    public function afterSaveTag(Varien_Event_Observer $o) {
        ...
        if ( array_key_exists($tagId, $this->oldData) 
        && $this->oldData[$tagId]['status'] == Mage_Tag_Model_Tag::STATUS_PENDING
        && $tag->getTag()->getStatus() == Mage_Tag_Model_Tag::STATUS_APPROVED ) {
            $tag->approvePendingTransfers();
        } elseif ( array_key_exists($tagId, $this->oldData) 
        && $this->oldData[$tagId]['status'] == Mage_Tag_Model_Tag::STATUS_PENDING 
        ...
        } elseif ( $tag->getTag()->getTagId() && ! isset($this->oldData[$tagId]['tag_id']) ) {
            $tag->ifNewTag();
        }
        return $this;
    }
