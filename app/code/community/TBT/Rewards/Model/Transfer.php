...
class TBT_Rewards_Model_Transfer extends TBT_Rewards_Model_Abstract
{
    ...
    public function _afterSave()
    {
        ...
        if ($this->_isNew) {
            ...
            $ref->setRewardsTransferId($this->getId());
            $ref->setId($this->_data['reference_data']['id']);
            $ref->setReferenceId($this->getReferenceId());
            $ref->setReferenceType($this->getReferenceType());
            $ref->setRuleId($this->_data['reference_data']['rule_id']);
            $ref->save();
            ...
            $this->setSourceReferenceId($refId)
                ->save();
    ...
    protected function _clearReferences()
    {
        $this->clearReferences();
        if ($this->_references) {
            foreach ($this->_references as $model) {
