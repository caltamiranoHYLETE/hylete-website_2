    ...
    protected function _importCampaign($tmp_fn)
    {
        try {
            ...
            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('rewards')->__('All MageRewards rules were imported successfully.')
            );

            if (Mage::getSingleton('rewards/migration_import')->getIsCatalogRuleImported()) {
                Mage::app()->saveCache(1, 'catalog_rules_dirty');
                Mage::getSingleton('adminhtml/session')->addNotice(
                    Mage::helper('rewards')->__('You must still APPLY MageRewards catalog earning/spending rules before they will take effect!')
                );
            }
        } catch (Exception $e) {
