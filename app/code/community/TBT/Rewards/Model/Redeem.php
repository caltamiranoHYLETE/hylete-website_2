...
class TBT_Rewards_Model_Redeem extends Mage_Core_Model_Abstract
{
    ...
    public function refactorRedemption(&$persistentItem, $doSave = true)
    {
        ...
        $this->applyPayPalExpressTaxAdjustments($item);
     ...
     * @param Mage_Sales_Model_Quote_Item $item
     */
    public function applyPayPalExpressTaxAdjustments($item)
    {
    ...
    /**
     * Reset rounding delta array
     */
    public function resetDeltas()
    {
        $this->_roundingDeltas = array();
        $this->_calculatedTaxAmounts = array();
        return $this;
    }
    ...
    protected function getUpdatedRedemptionData($item, $do_incl_tax = true)
    {
        ...
        if ($this->_taxHelper->priceIncludesTax()) {
            $row_total = $this->_deltaRound($row_total, 'default', $do_incl_tax, 'row_total');
            ...
            $row_total_incl_tax = $this->_deltaRound($row_total_incl_tax, 'default', $do_incl_tax, 'row_total');
        }
