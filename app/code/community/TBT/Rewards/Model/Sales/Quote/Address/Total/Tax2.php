...
class TBT_Rewards_Model_Sales_Quote_Address_Total_Tax2 extends Mage_Tax_Model_Sales_Total_Quote_Tax
{
    ...
    public function collect141(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        $this->addPayPalExpressAdjustments($address);
     ...
     * @param   Mage_Sales_Model_Quote_Address $address
     */
    public function addPayPalExpressAdjustments(Mage_Sales_Model_Quote_Address $address)
    {
        $redeemModel = Mage::getModel('rewards/redeem');
        $items = $this->_getAddressItems($address);
        
        // Reset RowTotalBeforeRedemptions 
        foreach ($items as $item) {
            $redeemModel->resetBeforeDiscount($item);
            $redeemModel->resetItemDiscounts($item);
        }
        
        // This will make sure that RowTotal & RowTotalInclTax are correct
        Mage::getModel('rewards/sales_quote_address_total_rewards')->getFinalPrice($address);
    }
