...
class TBT_Rewards_Model_Newsletter extends Mage_Newsletter_Model_Subscriber {
	...
	public function transferNewsletterPoints($rule) {
		...
		$rule_id = $rule->getId ();
		$transfer = $this->_getTransferHelper ()->initTransfer ( $num_points, $rule_id );
		...
		if (! $transfer->setStatusId ( null, Mage::helper ( 'rewards/config' )->getInitialTransferStatusAfterNewsletter () )) {
