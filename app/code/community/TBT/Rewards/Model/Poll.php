...
class TBT_Rewards_Model_Poll extends Mage_Poll_Model_Poll
{
    public function addVote(Mage_Poll_Model_Poll_Vote $vote)
    {
        if (Mage::getModel ( 'customer/session' )->getCustomerId ()) {
            ...
            $transferCollection = Mage::getModel ( 'rewards/poll_transfer' )->getTransfersAssociatedWithPoll ( $this->getId () );
            ...
            if (! $hasRecievedPoints) {
                ...
                foreach ( $ruleCollection as $rule ) {
                    ...
                    try {
                        $is_transfer_successful = Mage::helper ( 'rewards/transfer' )->transferPollPoints ( $rule->getPointsAmount (), $this->getId (), $rule->getId () );
                    } catch ( Exception $ex ) {
