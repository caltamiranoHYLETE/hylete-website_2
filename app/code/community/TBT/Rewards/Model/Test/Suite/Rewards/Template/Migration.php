    ...
    protected function generateSummary() {
        ...
        if (Mage::helper('rewards/version')->isBaseMageVersionAtLeast('1.4.0.0')) {
            if($hasRewardsFilesInDefault) {
                $this->addFail($this->__('Sweet Tooth template files should be in base/default not default/default'), $this->__('Help can be found here: http://help.sweettoothrewards.com/article/567-additional-sweet-tooth-1-6-update-steps'));
            ...
            if(!$hasRewardsFilesInDefault && $hasRewardsFilesInBase) {
                $this->addWarning($this->__('Sweet Tooth template files should be in default/default'), $this->__('Help can be found here: http://help.sweettoothrewards.com/article/567-additional-sweet-tooth-1-6-update-steps'));
            ...
            if($hasRewardsFilesInDefault && $hasRewardsFilesInBase) {
                $this->addWarning($this->__('Sweet Tooth template files in base/default will be ignored.'), $this->__('Help can be found here: http://help.sweettoothrewards.com/article/567-additional-sweet-tooth-1-6-update-steps'));
            }
        }
    }
