...
class TBT_Rewards_Model_Observer_Block_Output extends Varien_Object
{
    ...
    public function appendCartPointsSpender($block, $transport)
    {
        ...
        if (
            ($block->getBlockAlias () == 'coupon'
                || $block->getBlockAlias () == 'review'
                || $block->getBlockAlias () == 'summary')
            && $block->getChild('points.spender')
        ) {
            $html = $transport->getHtml();
            
            $sweettoothHtml = $block->getChildHtml('points.spender');

            // Check that content is not already integrated.
            if ($sweettoothHtml && strpos($html, $sweettoothHtml) === false) {
                $html = $sweettoothHtml . $html;
            }
