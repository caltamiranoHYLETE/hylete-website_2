...
class TBT_Rewards_Model_Observer_Sales_Convert_Quotetoorder implements TBT_Rewards_Model_Customer_Listener
{
    ...
    public function copyQuoteRewardsDataToOrder($quote, $order)
    {
        ...
        $order->setRewardsCartDiscountMap($quote->getRewardsCartDiscountMap());
    ...
    public function savePointsInfoInOrder($observer)
    {
        ...
        if ($this->_resetDeltas) {
            $redeemModel->resetDeltas();
            $this->_resetDeltas = false;
        }
        ...
        $orderItem->setRewardsCartDiscountMap($quoteItem->getRewardsCartDiscountMap());
    ...
    /**
     * Recalculate hidden tax order totals if for some reason it was reset to 0
     * and quote items have hidden tax gratter than 0
     * @param Mage_Sales_Model_Quote $quote
     * @param Mage_Sales_Model_Quote $order
     * @see `sales_convert_quote_address_to_order` event
     * @return \TBT_Rewards_Model_Observer_Sales_Convert_Quotetoorder
     */
    public function checkHiddenTax($observer)
    {
        ...
        if (!Mage::helper ( 'rewards/version' )->isMageVersionAtLeast ( '1.6.0.0' )) {
            return $this;
        }
        ...
        if ($order->getHiddenTaxAmount() > 0 || $order->getBaseHiddenTaxAmount() > 0) {
            return $this;
        }
        ...
        foreach ($quote->getAllItems() as $item) {
            $baseHiddenTax += $item->getBaseHiddenTaxAmount();
            $hiddenTax += $item->getHiddenTaxAmount();
        }
        
        $order->setHiddenTaxAmount($hiddenTax);
        $order->setBaseHiddenTaxAmount($baseHiddenTax);
