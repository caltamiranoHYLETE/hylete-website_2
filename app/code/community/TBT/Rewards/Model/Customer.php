...
class TBT_Rewards_Model_Customer extends Mage_Customer_Model_Customer
{
    ...
    protected $_isPointsBalanceLoaded      = false;
    protected $_pointsIndexModel   = null;
     ...
     * @deprecated use loadPointsBalance()
     * @see TBT_Rewards_Model_Customer::loadPointsBalance()
     *
     */
    private function _loadPointsCollections()
    {
        return $this->loadPointsBalance();
    }
     ...
     * @return TBT_Rewards_Model_Customer
     */
    public function loadPointsBalance()
    {
        try {
            if (empty($this->_pointsIndexModel) && !Mage::helper('rewards/customer_points_index')->useIndex()) {
                throw new Exception("Points indexer not available!");
            }

            $indexModel = $this->getIndexedPoints();
            $currencyId = Mage::helper('rewards/currency')->getDefaultCurrencyId();

            $this->usable_points = array( $currencyId => (int) $indexModel->getData('customer_points_usable'));
            $this->points = array( $currencyId => (int) $indexModel->getData('customer_points_active'));
            $this->on_hold_points = array( $currencyId => (int) $indexModel->getData('customer_points_pending_approval'));
            $this->pending_points = array( $currencyId => (int) $indexModel->getData('customer_points_pending_event'));
            $this->pending_time_points = array( $currencyId => (int) $indexModel->getData('customer_points_pending_time'));
        ...
        $this->_isPointsBalanceLoaded = true;
        return $this;
    }

    /**
     * @return bool true if points balance has been loaded into the model already, false otherwise.
     */
    public function isPointsBalanceLoaded()
    {
        return $this->_isPointsBalanceLoaded;
    }
    ...
    /**
     * Will set the indexed points model for this customer
     * @param TBT_Rewards_Model_Customer_Indexer_Points $indexModel
     * @return TBT_Rewards_Model_Customer
     */
    public function setIndexedPoints($indexModel = null)
    {
        $this->_pointsIndexModel = $indexModel;
        return $this;
    }

    /**
     * Will return the indexed points model for this customer
     * @throws Exception if no indexer found
     * @return TBT_Rewards_Model_Customer_Indexer_Points
     */
    public function getIndexedPoints()
    {
        if (!($this->_pointsIndexModel instanceof TBT_Rewards_Model_Customer_Indexer_Points)) {
            $this->_pointsIndexModel = Mage::getModel('rewards/customer_indexer_points')->load($this->getId());
            if (!$this->_pointsIndexModel->getId()) {
                throw new Exception("No customer record found in rewards index table for customer #{$this->getId ()}");
            }
        }

        return $this->_pointsIndexModel;
    }

    /**
     * Finds indexed balance data for this customer and returns the data as an array
     * @deprecated, use getIndexedPoints() instead
     * @see TBT_Rewards_Model_Customer::getIndexedPoints()
     * @return array, Data from TBT_Rewards_Model_Mysql4_Customer_Indexer_Points model
     */
    protected function _getIndexedCustomerBalanceData()
    {
        $indexedPoints = $this->getIndexedPoints();
        return $indexedPoints->getData();
    }
    ...
    /**
     * Returns the last time the user earned/spent points.
     * @return string date
     */
    ...
    /**
     * Will set all points of this customer to expire
     * @deprecated use TBT_Rewards_Model_Expiry::expireCustomerPoints() instead.
     * @see TBT_Rewards_Model_Expiry::expireCustomerPoints()
     */
    public function expireAllPoints()
    {
        Mage::getModel('rewards/expiry')->expireCustomerPoints($this);
        return $this;
    }
    ...
    /**
     * Garbage collection assistance in clearing child objects
     * @return $this
     */
    protected function _clearReferences()
    {
        foreach ($this->transfers as $transfer) {
            if ($transfer instanceof Mage_Core_Model_Abstract) {
                $transfer->clearInstance();
            }
        }

        if ($this->_pointsIndexModel instanceof Mage_Core_Model_Abstract) {
            $this->_pointsIndexModel->clearInstance();
        }
    ...
    /**
     * Garbage collection assistance in clearing instance data
     * @return $this
     */
    protected function _clearData()
    {
        $this->points = array();
        $this->on_hold_points = array();
        $this->pending_points = array();
        $this->pending_time_points = array();
        $this->usable_points = array();
        $this->transfers = array();
        $this->_isPointsBalanceLoaded = false;
        $this->_pointsIndexModel = null;
