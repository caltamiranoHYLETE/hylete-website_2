...
class TBT_Rewards_Model_Session extends Mage_Core_Model_Session_Abstract {
    ...
    /**
     * Returns the number of points being spent on the cart with catalog rules
     *
     * @return array() currency_id => points_qty
     */
    public function getCatalogPointsSpentOnCart($cart = null)
    {
        if ($cart == null) {
            $cart = $this->getQuote();
        }
    ...
    /**
     * Check if any points are spent with shopping cart rules
     * 
     * @param Mage_Sales_Model_Quote $cart
     * @return bool
     */
    public function areAnyPointsSpentOnShoppingCartRules($cart = null)
    {
        if ($cart == null) {
            $cart = $this->getQuote();
        }
