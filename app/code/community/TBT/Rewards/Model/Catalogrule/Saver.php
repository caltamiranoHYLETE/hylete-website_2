...
class TBT_Rewards_Model_Catalogrule_Saver extends Mage_Core_Model_Abstract 
{
    ...
    private function updateRedeemedPointsHash($date, $wId, $gId, $pId, $item, $apply_rule_id, $qty, $adjustQty = true, $uses = 1, $overwrite = false) 
    {
        ...
        $avail_extra_applic = $item->getQty () - $num_products_currently_affected + (int)$qty;
            ...
            if ($same_rule_id) {
                if ($overwrite) {
                    ...
                    if ($adjustQty) {
                        ...
                        $redeemedPointsQty = (array) $redeemed_points[self::APPLICABLE_QTY];
                        $new_applic_qty = array_pop($redeemedPointsQty) 
        ...
        foreach ( $collection as $row ) {
            ...
            foreach ( $associated_rule_ids as $rule_id ) {
                ...
                if (! isset ( $is_redemption_rule [$rule_id] )) {
                    // TODO Sweet Tooth: change this to use catalogrule_actions
                ...
                if ($is_redemption_rule [$rule_id]) {
                    // TODO Sweet Tooth: any way to optimize this array_search?
                    if (array_search ( $customer_group_id, $rule->getCustomerGroupIds () ) !== false) {
                        /* TODO Sweet Tooth - validate that this rule exists within the current website */
