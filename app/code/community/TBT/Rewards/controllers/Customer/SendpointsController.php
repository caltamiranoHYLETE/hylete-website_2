    ...
    public function sendAction() 
    {
        try {
            ...
            if (!Mage::getSingleton('customer/session')->isLoggedIn()) {
                throw new Exception('You must be logged in to transfer your points.');
            }
            ...
            if (!$friend_email || $friend_email === '') {
                ...
                $sender = Mage::getSingleton('rewards/customer')->load($customerId);
                
                $currencyId = Mage::helper('rewards/currency')->getDefaultCurrencyId();
                $points_sent_string = Mage::getModel('rewards/points')->set($currencyId, $points_amt)->__toString();
                ...
                $balance = $sender->getUsablePointsBalance($currencyId);
                if (!$balance) {
                    $balance = 0;
                }
                
                if ($points_amt > $balance) {
                    Mage::getSingleton('core/session')->addError($this->__('You cannot send %s to your friend when you only have %s!', Mage::getModel('rewards/points')->set($currencyId, $points_amt), Mage::getModel('rewards/points')->set($currencyId, $balance)));
                ...
                /* TODO Sweet Tooth - point currencies are not yet specific to customer groups */
                if (!$friend->hasCurrencyId($currencyId)) {
                    $currency_caption = Mage::getSingleton('rewards/currency')->getCurrencyCaption($currencyId);
                ...
                $is_transfer_successful = Mage::helper('rewards/transfer')->transferPointsToFriend($points_amt, $friend_id, $personal_comment);
    ...
    public function notifyFriendEmail($customer, $friend, $pointsString, $personal_comment='', $invitation = false) 
    {
        ...
        if ($invitation) {
            $vars = array(
                ...
                'points_transferred' => Mage::helper('rewards')->getPointsString($pointsString),
            ...
            $result = $emailHelper->sendTransactional($template, $sender, $friend, $vars);
        }
