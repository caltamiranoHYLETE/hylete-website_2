...
abstract class TBT_Rewards_Block_Manage_Promo_Quote_Edit_Tab_Abstract extends Mage_Adminhtml_Block_Widget_Form {
	...
	protected function _getPriceActionFieldset(Varien_Data_Form &$form, TBT_Rewards_Model_Salesrule_Rule &$model) {
		...
		if ($this->_isRedemptionType ()) {
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "article/429-setting-up-a-shopping-cart-spending-rule", "Shopping Cart Spending Rule - Actions - Update prices using the following information" );
...
*/
			...
			$disabled_field_msg_html = "<div class='disabled-field-msg' style='font-style: italic; font-size: 10px;'>" . $this->__ ( "This feature has been temporarily disabled for this version of MageRewards in ensure quality.  It will be re-enabled in the next release of MageRewards." ) . "</div>";
			//@nelkaake -a 16/11/10: 
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($stop_rules_processing_field, "article/400-stop-further-rule-processing", "Shopping Cart Spending Rule - Actions - Stop Further Rules Processing" );
	...
	protected function _getApplyToActionFieldset(&$form, &$model) {
		...
		Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "article/428-how-to-apply-the-rule-actions-to-a-subset-of-products", "How to apply the rule actions to a subset of products" );
	...
	protected function _getPointsActionFieldset(&$form) {
		...
		if ($this->_isRedemptionType ()) {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "article/429-setting-up-a-shopping-cart-spending-rule", "Shopping Cart Spending Rule - Conditions" );
		} else {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "article/403-setting-up-a-shopping-cart-earning-rule", "Shopping Cart Earning Rule - Actions" );
		}
		...
		if ($this->_isRedemptionType ()) {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($points_max_qty_field, "article/303-maximum-distributed-points", "Shopping Cart Points Spending Rule: Maximum Distributed Points" );
		} else {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($points_max_qty_field, "article/303-maximum-distributed-points", "Shopping Cart Points Earning Rule: Maximum Distributed Points" );
		}
